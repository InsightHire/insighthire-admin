  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        {/* Careers page (read-only) — read off the cultureSummary
            shape so we don't have to fan out a second query. Placed
            above the summary cards so platform support sees public
            visibility state at a glance, before any of the internal
            scoring data. */}
        <CareersPageSection
          status={status}
          careersPage={summary?.careersPage}
        />
...
/**
 * Read-only Careers Page section for the platform-admin org culture
 * detail page. Mirrors the org-side card on /admin/culture/careers but
 * without any mutation surface — platform admins must use Login as
 * Admin to actually toggle publish state, since the underlying
 * `culture.org.setCareersPublished` mutation is org-admin gated.
 *
 * Accepts the careersPage shape that platform-admin `cultureSummary`
 * returns (added in the same change). When `careersPage` is missing
 * (loading) we render a thin placeholder; when culture is disabled
 * org-wide we still show the section but with an "N/A" pill so the
 * platform admin knows publish state is moot until the org master
 * toggle flips on.
 */
function CareersPageSection({
  ...
  status: CultureStatus | undefined;
  careersPage:
    | {
        published: boolean;
        url: string | null;
  ...
  // The cultureSummary `careersPage.published` already AND-gates on the
  // org master toggle (`profile.enabled`), so a mismatch between the
  // toggle and `publishedAt` resolves to "not published" here. We
  // additionally surface an "N/A" pill when the org has nothing
  // configured at all so the platform admin knows there's nothing to
  // publish in the first place.
  const isCultureOff =
    status === 'PLATFORM_OFF' ||
    status === 'NOT_GRANTED' ||
    status === 'GRANTED_NOT_CONFIGURED';

  const published = careersPage?.published === true;
  const publishedAt = careersPage?.publishedAt ?? null;
  const url = careersPage?.url ?? null;

  let pillLabel = 'Loading…';
  let pillClass = 'bg-gray-100 text-gray-700 ring-gray-200';
  let pillHelp = '';
  if (careersPage) {
    if (published) {
      pillLabel = 'Published';
      pillClass = 'bg-emerald-100 text-emerald-800 ring-emerald-200';
      pillHelp = 'Live at the public URL below.';
    } else if (isCultureOff) {
      pillLabel = 'N/A';
      pillClass = 'bg-gray-100 text-gray-700 ring-gray-200';
      pillHelp =
        'Culture-fit scoring is not enabled for this org — there is no profile to publish.';
    } else {
      pillLabel = 'Not published';
      pillClass = 'bg-gray-100 text-gray-700 ring-gray-200';
      pillHelp =
        'Org admin has not flipped the careers publish toggle on yet.';
    }
  }
  ...
  const publishedRelative =
    publishedDate && !Number.isNaN(publishedDate.getTime())
      ? formatRelative(publishedDate)
      : '';

  return (
    <section
      id="careers"
      className="bg-white rounded-xl shadow-sm border border-gray-200/60 overflow-hidden scroll-mt-20"
    >
      <header className="px-6 py-4 bg-gradient-to-r from-slate-50 to-gray-50 border-b border-gray-200">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            <h2 className="text-lg font-semibold text-gray-900">Careers Page</h2>
            <span
              title={pillHelp}
      ...
      <div className="p-6 space-y-4">
        {published && url ? (
          <>
            <div className="rounded-md border border-emerald-200 bg-emerald-50/60 p-3">
              ...
              <code className="mt-1 block truncate rounded-md border border-emerald-200 bg-white px-3 py-2 text-xs font-mono text-emerald-900">
                {url}
              </code>
              {publishedDate && (
                <p className="mt-2 text-[11px] text-emerald-800/80">
                  Last published {publishedRelative} ·{' '}
                  {publishedDate.toLocaleString()}
                </p>
              )}
            </div>

            {/* Read-only iframe preview. We sandbox to disable scripts
                entirely — this is a passive view-only render in an
                admin context, we don't want client JS from the
                customer app running here. allow-same-origin is
                included so styles and image fetches resolve; without
                it, fonts fail. */}
            ...
            <p className="text-xs text-gray-500">
              Sandboxed preview — scripts are disabled. Open in a new
              tab to interact normally.
            </p>
          </>
        ) : isCultureOff ? (
          <div className="rounded-md border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-sm text-gray-600">
            Culture-fit scoring isn&apos;t enabled for this organization,
            so the careers page can&apos;t be published. The org admin
            needs to enable culture and configure a profile first.
          </div>
        ) : (
          <div className="rounded-md border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-sm text-gray-600">
            The org has not published a careers page yet. Once their
            admin flips the publish toggle, a candidate-facing summary
            of the culture profile becomes live at{' '}
        ...
        <div className="rounded-md border border-amber-200 bg-amber-50/60 p-3 flex items-start gap-2">
          <InformationCircleIcon className="h-5 w-5 text-amber-700 mt-0.5 shrink-0" />
          <p className="text-xs text-amber-900">
            Platform admins cannot publish on behalf of an org. To
            toggle, use <strong>Login as Admin</strong> above and visit{' '}
            <code className="rounded bg-amber-100 px-1 py-0.5 font-mono">
              /admin/culture/careers
            </code>
            .
          </p>
        </div>
...
/** Format a "X minutes/hours/days ago" relative-time label. */
